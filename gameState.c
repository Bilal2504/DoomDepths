...
int insertPlayer(sqlite3 *db, st_player* p_player, st_level* p_level, int saveId)
{
    ...
    snprintf(joueur_insert_sql, sizeof(joueur_insert_sql),
             "INSERT INTO JOUEUR (maxLife, currentLife, attack, gold, maxMana, currentMana, id_niveau, id_sauvegarde) VALUES (%d, %d, %d, %d, %d, %d, %d, %d);",
             p_player->maxLife, p_player->currentLife, p_player->maxAttack, p_player->gold, p_player->maxMana, p_player->currentMana, p_level->lvl_number, saveId);
