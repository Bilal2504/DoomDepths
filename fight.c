...
st_monsters *fight_player_round (st_player *p_player, st_monsters *p_first_monster, int nb_level)
{
    ...
    p_monster_found->currentLife -= p_player->attack; //randomDamage; // opération, on diminue la vie du monstre, en fonction de l'attaque du joueur.
    ...
    { // si la vie du monstre == 0
        ...
        if (dropChance <= 25)
        {
            ...
            if (whatEquipment == 1)
            {
                ...
                addWeaponsPlayer(p_player, newWeapon);
            }
            else
            {
                Armor *newArmor = createArmor(nb_level);
                addArmorsPlayer(p_player, newArmor);
            }
        }
